...
#'
#' @description Deprecated as of 2024-07-18. Use \link{clean_resp_support} with
#'   \code{var_col = 'flowsheet_measure_name'} and an appropriate \code{var_map}.
#'   In older Epic data (pre-7/18/2024), measurements are identified by
#'   \code{flowsheet_measure_name} (character) rather than \code{flowsheet_measure_id}
#'   (numeric). See \code{inst/scripts/resp_support_map_old.R} for the full name mapping.
#'
#' @param df_resp A long-form data frame of respiratory settings and measurements
#'
#' @examples
#' \dontrun{
#' # Source the map from inst/scripts/resp_support_map_old.R, then:
...
clean_resp_support_old <- function(df_resp) {
     .Deprecated("clean_resp_support",
                 msg = paste("clean_resp_support_old() is deprecated.",
                             "Use clean_resp_support() with var_col = 'flowsheet_measure_name'",
                             "and a var_map matching your flowsheet measure names.",
                             "See ?clean_resp_support_old for the full map."))
}
